...
if __name__ == "__main__":
    ser = init_uart()  # UART 초기화

    try:
        while True:
            data_to_send = "Hello from Slave"  # 실제 데이터가 없으면 None으로 설정 가능
            received_message = slave_communication(ser, data_to_send)  # 메시지 수신 및 응답 전송
            time.sleep(2)  # 2초 간격으로 데이터 송수신
    except KeyboardInterrupt:
        close_uart(ser)  # 종료 시 UART 포트 닫기
